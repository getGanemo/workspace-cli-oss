...
`wsp` is the reference CLI for **Agent Workspace as Code (AWaC)** — a method for composing AI-agent workspaces declaratively from versioned stack repos, so every developer (and every AI coding agent) gets the same rules, skills, and workflows. No copy-paste, no drift, no bloat.
...
stacks:
  ...
  - my-org/agent-stack    # your project's stack
```

Run `wsp bootstrap`, get `.agents/{rules,skills,workflows}/` composed in deterministic order, your project repos cloned, your `CLAUDE.md` and `AGENTS.md` generated. Lockfile records exact commits. Idempotent.

## Install

Requirements: Python ≥ 3.10, `git`, `pipx`, and `gh` (GitHub CLI logged in via `gh auth login`).
...
- Project repos declared by the stacks are cloned into the workspace.
...
| `wsp sync` | Refresh stacks and recompose `.agents/`. Does not touch project repos. |
...
| `wsp scaffold-repo <full> --category <A\|B\|C\|D\|E>` | Create or audit a project-repo README. |
...
| `wsp governance check` | Compare local `awac.yml#org_scaffold` mirror against your team's governance source. |
...
- **Permissive license**: MIT. Fork it, vendor it, embed it in your tooling.

## Documentation

- **Docs site**: <https://awac.ganemo.com>
- **Docs source**: <https://github.com/getGanemo/awac-docs>
- **CHANGELOG**: [CHANGELOG.md](CHANGELOG.md)

## Reference stacks

Public OSS reference stacks. Compose them via `workspace.yml/stacks` to get them auto-cloned and composed:

- [`getGanemo/agent-stack-core-oss`](https://github.com/getGanemo/agent-stack-core-oss) — universal foundation: registry of shortcuts, templates, universal rules and skills.
- [`getGanemo/agent-stack-aws-oss`](https://github.com/getGanemo/agent-stack-aws-oss) — AWS safety + Terraform parity patterns.
...
PRs welcome. See [CONTRIBUTING.md](CONTRIBUTING.md) for dev setup, conventions, and what we are looking for.
...
AWaC was created by [Fernando Pastor](https://github.com/GanemoCorp) at [Ganemo Corp](https://www.ganemo.com). The design was informed by working day-to-day with [Anthropic Claude](https://www.anthropic.com/claude) and the broader AI coding ecosystem (Cursor, Aider, Continue).
